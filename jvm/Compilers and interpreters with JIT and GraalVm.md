...
## Graal Native image and dynamic features 
...
https://www.graalvm.org/latest/reference-manual/native-image/dynamic-features/


Java supports many dynamic features that allow to **change the application classes and methods at run-time**, invalidating the close world assumption. 

More details of dynamic features on [Dynamic features doc](<Dynamic features of Java.md>)

* *Dynamic class loading*: loading classes from run time sources 
* *Proxies*: method calls redirected to proxies at run-time
* *Reflection*: class instanciation/method invocation at run time
* *invokedynamic*: efficient lambda expression resolution at run-time 
* *JNI*: dynamic native code linking and execution
